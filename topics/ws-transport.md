...
2. It sends `mod.apphost.route_query_msg` with a fresh 16-hex-character
   `Nonce`, `Caller`, `Target`, `Query` (with `?args` merged in), `Zone`
   and `Filters`.
...
4. After acceptance, both peers exchange `AstralObject` frames freely. The
   stream ends at the first `eos` frame or when the WS closes; the async
...
3. Server replies with `ack`; on error it sends `mod.apphost.error_msg`
   (e.g. unauthorized). Authorization mirrors `route_query`: the caller
   identity must equal `Identity` or hold a `SudoAction` for it.
